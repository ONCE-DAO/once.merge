...
export default abstract class OnceKernel {
  ...
  static async discover(): Promise<Once> {
    ...
    if (this.RuntimeIs.NODE_LOADER()) {
      return (
        await import("../2_systems/Once/OnceNodeImportLoader.class.mjs")
      ).default.start();
    }
    if (this.RuntimeIs.NODE_JS()) {
      return (
        await import("../2_systems/Once/OnceNodeServer.class.mjs")
      ).default.start();
    }
    if (this.RuntimeIs.BROWSER()) {
      return (
        await import("../2_systems/Once/OnceBrowser.class.mjs")
      ).default.start();
    }
